      ...
      <div className="max-w-7xl mx-auto space-y-20 relative z-10">
        <div className="space-y-6 text-center">
          ...
          <motion.p 
            ...
            className="max-w-2xl text-[12px] sm:text-sm text-muted-foreground font-bold uppercase tracking-[0.2em] leading-relaxed mx-auto opacity-70"
          >
            Full Stack software systems engineered for performance and architectural stability.
          </motion.p>
        </div>
          ...
          {projects.map((project, idx) => (
            ...
            >
              <Link href={`/projects/${project.id}`} className="block h-full group">
                <Card className="h-full bg-secondary border-border hover:border-primary transition-all rounded-xl overflow-hidden shadow-lg">
                  <CardContent className="p-8 space-y-8 flex flex-col h-full">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary bg-primary/10 px-3 py-1 rounded">
                        {project.category}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[9px] font-black uppercase tracking-widest px-3 py-1 bg-background border border-border rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="pt-6 border-t border-border flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">Initialize Sync</span>
