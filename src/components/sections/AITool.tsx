      ...
      <div className="max-w-6xl mx-auto space-y-10 sm:space-y-16 relative z-10">
        ...
        <Tabs defaultValue="narrative" className="w-full" onValueChange={setActiveTool}>
          <div className="flex justify-center mb-8 sm:mb-14">
            <TabsList className="bg-secondary/10 p-1 rounded-xl h-14 sm:h-20 border-none backdrop-blur-md">
              <TabsTrigger 
                value="narrative" 
                className="px-6 sm:px-16 rounded-lg font-black uppercase text-[12px] sm:text-sm tracking-[0.2em] h-full data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-[0_0_20px_rgba(var(--primary),0.2)] transition-all border-none"
              >
                NARRATIVE
              </TabsTrigger>
              <TabsTrigger 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12">
            <motion.div initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <TabsContent value="narrative" className="m-0">
                <Card className="glass-card border-none bg-card/40 rounded-[1.5rem] sm:rounded-3xl overflow-hidden shadow-2xl group transition-all duration-500">
                  <CardContent className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                    <div className="flex items-center gap-3">
                      <Terminal className="w-5 h-5 text-primary" />
                      <h3 className="text-base sm:text-xl font-black uppercase tracking-tight text-primary">System Params</h3>
                    </div>
                    <form onSubmit={handleNarrative} className="space-y-4 sm:space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-2">Tech Stack</label>
                        <Input 
                          ...
                          className="h-12 sm:h-14 bg-background/40 border-none rounded-lg font-black text-xs uppercase tracking-widest px-4 sm:px-5 focus:bg-background/60 transition-all text-primary"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-2">Project Scope</label>
                        <Textarea 
                          placeholder="Define the mission..." 
                          value={narrativeData.scope}
                          onChange={(e) => setNarrativeData({...narrativeData, scope: e.target.value})}
                          className="min-h-[100px] sm:min-h-[140px] bg-background/40 border-none rounded-lg p-4 sm:p-5 resize-none font-bold text-xs uppercase tracking-tight focus:bg-background/60 transition-all text-primary"
                        />
                      </div>
                      <Button type="submit" className="w-full h-14 sm:h-16 rounded-lg font-black uppercase tracking-widest text-xs shadow-lg bg-primary text-primary-foreground hover:shadow-[0_0_30px_rgba(var(--primary),0.5)] hover:scale-[1.02] active:scale-95 transition-all duration-300 border-none" disabled={loading}>
                ...
                <Card className="glass-card border-none bg-card/40 rounded-[1.5rem] sm:rounded-3xl overflow-hidden shadow-2xl group transition-all duration-500">
                  <CardContent className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base sm:text-xl font-black uppercase tracking-tight text-primary">Brand Matrix</h3>
                    </div>
                    <form onSubmit={handleBrand} className="space-y-4 sm:space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-2">Entity Name</label>
                          <Input 
                            placeholder="Name" 
                            value={brandData.name}
                            onChange={(e) => setBrandData({...brandData, name: e.target.value})}
                            className="h-12 sm:h-14 bg-background/40 border-none rounded-lg font-black text-xs uppercase px-4 focus:bg-background/60 transition-all text-primary"
                          />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-2">Tone</label>
                          <select 
                            value={brandData.tone}
                            onChange={(e) => setBrandData({...brandData, tone: e.target.value as any})}
                            className="w-full h-12 sm:h-14 bg-background/40 border-none rounded-lg px-3 font-black text-[10px] uppercase tracking-widest focus:outline-none focus:bg-background/60 transition-all text-primary"
                          >
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-2">Mission</label>
                        <Textarea 
                          placeholder="Brand purpose..." 
                          value={brandData.mission}
                          onChange={(e) => setBrandData({...brandData, mission: e.target.value})}
                          className="min-h-[100px] sm:min-h-[120px] bg-background/40 border-none rounded-lg p-4 sm:p-5 font-bold text-xs uppercase focus:bg-background/60 transition-all text-primary"
                        />
                      </div>
                      <Button type="submit" className="w-full h-14 sm:h-16 rounded-lg font-black uppercase tracking-widest text-xs shadow-lg bg-primary text-primary-foreground hover:shadow-[0_0_30px_rgba(var(--primary),0.5)] hover:scale-[1.02] active:scale-95 transition-all duration-300 border-none" disabled={loading}>
            ...
            <motion.div initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <Card className="glass-card border-none bg-card/60 rounded-[1.5rem] sm:rounded-3xl overflow-hidden h-full flex flex-col shadow-2xl group transition-all duration-500">
                <CardContent className="p-6 sm:p-10 flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-6 sm:mb-8">
                    <div className="flex items-center gap-3">
                      <ShieldCheck className="w-5 h-5 text-primary" />
                      <h3 className="text-base sm:text-xl font-black uppercase tracking-tight text-primary">Output</h3>
                    </div>
                    {(narrativeResult || brandResult) && (
                      <Button variant="ghost" size="icon" onClick={() => copyToClipboard(narrativeResult?.projectDescription || brandResult?.professionalDescription || "")} className="h-10 w-10 rounded-lg border-none bg-background/20 hover:bg-primary/20 transition-all text-primary">
                      ...
                      {activeTool === "narrative" ? (
                        narrativeResult ? (
                          <motion.div key="narrative-out" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6 text-center">
                            <div className="p-6 sm:p-10 rounded-xl bg-background/50 border-none text-xs sm:text-base font-bold uppercase tracking-tight shimmer-text leading-relaxed hover:bg-background/70 transition-all text-primary">
                        ...
                        brandResult ? (
                          <motion.div key="brand-out" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                             <div className="p-6 sm:p-10 rounded-xl bg-background/50 border-none space-y-4 hover:bg-background/70 transition-all">
                               <p className="text-primary font-black text-[8px] uppercase tracking-[0.4em] opacity-50">Identity Verified</p>
                               <p className="text-xs sm:text-base font-bold uppercase tracking-tight leading-relaxed text-primary">{brandResult.professionalDescription}</p>
                             </div>
                             <div className="p-4 sm:p-6 rounded-lg bg-primary/5 border-none text-[10px] sm:text-[11px] font-black uppercase tracking-[0.4em] text-primary">
...
function EmptyState({ loading }: { loading: boolean }) {
  return (
    <div className="text-center space-y-4 py-8">
      ...
      <p className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.5em] opacity-30">
