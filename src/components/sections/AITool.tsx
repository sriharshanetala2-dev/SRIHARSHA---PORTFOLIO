      ...
      <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        <div className="flex flex-col items-center text-center space-y-8">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="px-6 py-2 rounded-full bg-primary/10 border border-primary/20 text-[11px] font-black text-primary uppercase tracking-[0.5em] flex items-center gap-3 shadow-lg"
          >
          ...
          <motion.p 
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-muted-foreground max-w-3xl text-sm sm:text-lg font-bold uppercase tracking-tight opacity-80"
          >
        ...
        <Tabs defaultValue="narrative" className="max-w-6xl mx-auto" onValueChange={setActiveTool}>
          <div className="flex justify-center mb-12 sm:mb-16">
            <TabsList className="bg-secondary/20 p-1.5 rounded-2xl h-14 sm:h-20 border border-border shadow-xl backdrop-blur-md">
              <TabsTrigger value="narrative" className="px-4 sm:px-12 rounded-xl font-black uppercase text-[10px] sm:text-[11px] tracking-widest data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-lg transition-all h-full">
                NEURAL NARRATIVE
              </TabsTrigger>
              <TabsTrigger value="brand" className="px-4 sm:px-12 rounded-xl font-black uppercase text-[10px] sm:text-[11px] tracking-widest data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-lg transition-all h-full">
                STRUCTURAL IDENTITY
              </TabsTrigger>
            </TabsList>
          </div>
                ...
                <Card className="glass-card border-border bg-card/5 rounded-[2rem] overflow-hidden shadow-2xl">
                  <CardContent className="p-8 sm:p-12 space-y-10">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight">System Params</h3>
                    </div>
                    <form onSubmit={handleNarrative} className="space-y-8">
                      <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-4">Tech Stack</label>
                        <Input 
                      ...
                      <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-4">Project Scope</label>
                        <Textarea 
                      ...
                      <Button type="submit" className="w-full h-16 rounded-full font-black uppercase tracking-widest text-[11px] shadow-xl bg-primary text-primary-foreground" disabled={loading}>
                ...
                <Card className="glass-card border-border bg-card/5 rounded-[2rem] overflow-hidden shadow-2xl">
                  <CardContent className="p-8 sm:p-12 space-y-10">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight">Brand Matrix</h3>
                    </div>
                    <form onSubmit={handleBrand} className="space-y-8">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-3">
                          <label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-4">Entity Name</label>
                          <Input 
                            placeholder="Name" 
                        ...
                        <div className="space-y-3">
                          <label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-4">Tone</label>
                          <select 
                            value={brandData.tone}
                            onChange={(e) => setBrandData({...brandData, tone: e.target.value as any})}
                            className="w-full h-16 bg-background/30 border border-border rounded-xl px-4 font-black text-[11px] uppercase tracking-widest focus:outline-none"
                          >
                      ...
                      <div className="space-y-3">
                        <label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground ml-4">Mission</label>
                        <Textarea 
                ...
                <CardContent className="p-8 sm:p-12 flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight">System Output</h3>
                    </div>
                      ...
                      {activeTool === "narrative" ? (
                        narrativeResult ? (
                          <motion.div key="narrative-out" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8 text-center">
                            <div className="p-8 rounded-[2rem] bg-background/40 border border-border text-sm sm:text-lg font-bold uppercase tracking-tight shimmer-text leading-relaxed">
                        ...
                        brandResult ? (
                          <motion.div key="brand-out" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                             <div className="p-8 rounded-[2rem] bg-background/40 border border-border space-y-4">
                               <p className="text-primary font-black text-[11px] uppercase tracking-widest opacity-50">Identity Verified</p>
                               <p className="text-sm sm:text-lg font-bold uppercase tracking-tight leading-relaxed">{brandResult.professionalDescription}</p>
                             </div>
                             <div className="p-6 rounded-xl bg-primary/5 border border-primary/10 text-[11px] font-black uppercase tracking-widest opacity-70">
...
function EmptyState({ loading }: { loading: boolean }) {
  return (
    <div className="text-center space-y-8 py-12">
      ...
      <p className="text-[11px] font-black uppercase tracking-[0.5em] opacity-30">
