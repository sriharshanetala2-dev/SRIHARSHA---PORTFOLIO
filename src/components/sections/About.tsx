        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
          ...
          >
            <div className="space-y-4 sm:space-y-6 text-center lg:text-left">
              <div className="inline-flex items-center gap-3 px-4 sm:px-5 py-2 rounded-full bg-primary/10 text-xs font-black tracking-widest text-primary uppercase border border-primary/20 mx-auto lg:mx-0">
                <Fingerprint className="w-4 h-4" />
                Developer Registry
              </div>
              <h2 className="text-4xl sm:text-5xl lg:text-6xl font-headline font-black leading-[1.1] tracking-tighter uppercase">
                CODE <br className="hidden sm:block" />
                <span className="text-primary">IDENTITY</span>
              </h2>
              <p className="text-sm sm:text-base lg:text-lg text-muted-foreground leading-relaxed font-bold max-w-xl mx-auto lg:mx-0 opacity-80 uppercase tracking-widest">
                ...
                <div className="p-6 sm:p-10 rounded-2xl sm:rounded-[2rem] border border-border/50 bg-secondary/5 space-y-8 sm:space-y-10 relative overflow-hidden shadow-xl">
                  <div className="flex flex-col items-center gap-4 sm:gap-6 relative z-10">
                    ...
                    <div className="text-center space-y-1">
                      ...
                      <h3 className="text-3xl sm:text-4xl font-headline font-black uppercase tracking-tighter">{activeNode.label}</h3>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 relative z-10">
                    {activeNode.metrics.map((metric, i) => (
                      <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-background border border-border shadow-sm">
                        ...
                        <span className="text-xs sm:text-sm font-black text-primary uppercase tracking-wider text-right">{metric}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="pt-6 border-t border-border/50 flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs font-black text-green-600 uppercase tracking-widest">{activeNode.status} Node</span>
                      </div>
                    </div>
