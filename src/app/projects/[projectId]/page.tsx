        ...
        <div className="max-w-7xl mx-auto space-y-12 sm:space-y-32">
          ...
          <div className="space-y-8 sm:space-y-16 max-w-5xl">
            ...
            <motion.h1 
              ...
              className="text-2xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl font-headline font-black tracking-tighter uppercase leading-[1.1] flex flex-wrap"
            >
          ...
          <motion.div 
            ...
            className="relative min-h-[500px] sm:aspect-video rounded-[2.5rem] sm:rounded-[4.5rem] overflow-hidden border border-border/60 shadow-3xl bg-secondary/5 data-flow-grid group"
          >
            <div className="absolute inset-0 flex items-center justify-center p-6 sm:p-20">
              ...
              <div className="relative z-10 flex flex-col items-center gap-8 sm:gap-20 w-full">
                ...
                >
                  ...
                  <div className="absolute -top-2 -right-2 sm:-top-8 sm:-right-8 p-3 sm:p-10 rounded-xl sm:rounded-[3rem] bg-accent text-accent-foreground shadow-3xl border border-white/10">
                    <Zap className="w-6 h-6 sm:w-16 sm:h-16 animate-pulse" />
                  </div>
                </motion.div>

                <div className="flex flex-col items-center gap-2 sm:gap-6">
                  <p className="text-[10px] sm:text-[13px] font-black uppercase tracking-[0.6em] text-primary opacity-60">System Core Node</p>
                  <h3 className="text-xl sm:text-5xl lg:text-7xl xl:text-8xl font-black uppercase tracking-tighter text-center px-4 leading-tight">{project.title}</h3>
                </div>

                {/* Metrics HUD - Fixed Horizontal 3-Column Persistent */}
                <div className="grid grid-cols-3 items-center justify-center gap-1 sm:gap-12 px-2 sm:px-20 py-6 sm:py-16 glass-card rounded-[2rem] sm:rounded-[5rem] border-white/10 bg-background/60 shadow-3xl backdrop-blur-2xl w-full max-w-5xl mx-auto">
                   {project.metrics.map((metric, i) => (
                     <div key={i} className="flex flex-col items-center px-1 sm:px-14 border-r last:border-0 border-border/30 overflow-hidden text-center">
                       <span className="text-[8px] sm:text-[13px] lg:text-[15px] font-black uppercase tracking-[0.1em] sm:tracking-[0.5em] text-muted-foreground mb-1 sm:mb-4 whitespace-nowrap">{metric.label}</span>
                       <span className="text-[10px] sm:text-3xl lg:text-5xl font-black text-accent uppercase font-headline whitespace-nowrap">{metric.value}</span>
                     </div>
                   ))}
                </div>
              </div>
            ...
            <div className="absolute top-4 left-4 sm:top-14 sm:left-14 flex gap-2 sm:gap-6">
               <div className="w-8 h-8 sm:w-20 sm:h-20 rounded-lg sm:rounded-[2.5rem] glass-card border-white/5 bg-black/40 flex items-center justify-center shadow-2xl">
                 <Cpu className="w-4 h-4 sm:w-10 sm:h-10 text-primary/60" />
               </div>
               <div className="w-8 h-8 sm:w-20 sm:h-20 rounded-lg sm:rounded-[2.5rem] glass-card border-white/5 bg-black/40 flex items-center justify-center shadow-2xl">
                 <ShieldCheck className="w-4 h-4 sm:w-10 sm:h-10 text-primary/60" />
               </div>
            </div>

            {/* Node Secure Badge */}
            <div className="absolute bottom-4 left-4 sm:bottom-14 sm:left-14 p-2 sm:p-10 glass-card rounded-xl sm:rounded-[3rem] border-white/10 bg-black/60 flex items-center gap-2 sm:gap-6 shadow-3xl">
              <div className="relative">
                <div className="w-2 h-2 sm:w-6 sm:h-6 rounded-full bg-green-500 animate-ping absolute inset-0" />
                <div className="w-2 h-2 sm:w-6 sm:h-6 rounded-full bg-green-500 shadow-[0_0_20px_#22c55e]" />
              </div>
              <span className="text-[9px] sm:text-sm lg:text-base font-black uppercase tracking-[0.3em] sm:tracking-[0.8em] text-green-500">Node Secure</span>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 sm:gap-32">
            <div className="lg:col-span-8 space-y-24 sm:space-y-40">
              <section className="space-y-8 sm:space-y-20">
                <h2 className="text-3xl sm:text-7xl lg:text-8xl font-headline font-black tracking-tight flex items-center gap-4 sm:gap-12 uppercase shimmer-text leading-tight">
